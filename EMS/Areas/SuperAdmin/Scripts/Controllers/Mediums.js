...
app.factory("EMSService", ['$http', '$window', function ($http, $window) {
    return {
        createData: function (Mediums) {
            $http({
                url: "/Mediums/CreateData",
                method: "POST",
                data: Mediums
            }).then(function (response) {
                if (response !== 'undefined' && typeof (response) == 'object') {
                    window.location.href = '/SuperAdmin/Mediums/Index';
                }
            }, function (response) {

            })
        },
        updateData: function (Mediums) {
            Mediums.ID = this.getEditId();
            $http({
                url: "/Mediums/UpdateData",
                method: "POST",
                data: Mediums
            }).then(function (response) {
                if (response !== 'undefined' && typeof (response) == 'object') {
...
app.controller('ListCtlr', ['$scope', '$http', '$window', 'EMSService', function ($scope, $http, $window, EMSService) {
    ...
    $scope.redirect = function (ID) {
        $window.location.href = '/SuperAdmin/Mediums/Edit/' + ID;
    };
