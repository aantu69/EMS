...
app.factory("EMSService", ['$http', '$window', function ($http, $window) {
    return {
        createData: function (Shifts) {
            $http({
                url: "/Shifts/CreateData",
                method: "POST",
                data: Shifts
            }).then(function (response) {
                if (response !== 'undefined' && typeof (response) == 'object') {
                    window.location.href = '/SuperAdmin/Shifts/Index';
                }
            }, function (response) {

            })
        },
        updateData: function (Shifts) {
            Shifts.ID = this.getEditId();
            $http({
                url: "/Shifts/UpdateData",
                method: "POST",
                data: Shifts
            }).then(function (response) {
                if (response !== 'undefined' && typeof (response) == 'object') {
...
app.controller('ListCtlr', ['$scope', '$http', '$window', 'EMSService', function ($scope, $http, $window, EMSService) {
    ...
    $scope.redirect = function (ID) {
        $window.location.href = '/SuperAdmin/Shifts/Edit/' + ID;
    };
...
app.controller('AddCtlr', ['$scope', '$http', 'EMSService', function ($scope, $http, EMSService) {
    ...
    $scope.addData = function () {
        var shift = {};
        shift.ShiftName = $scope.ShiftName;
        EMSService.createData(shift);
    };
}]);
